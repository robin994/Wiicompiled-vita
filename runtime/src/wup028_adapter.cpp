...
// The official WUP-028 adapter is driven directly over WinUSB, so this whole
// implementation is Windows-only. Everything below the #else is a stub that
// reports the adapter as permanently unavailable, so callers (main.cpp,
// hle/input/pad.cpp, settings_overlay.cpp) don't need platform checks of
// their own.
#ifdef _WIN32
...
AdapterInfo GetInfo() {
    ...
    info.detail = "WUP-028 GameCube adapter support requires Windows (WinUSB).";
    return info;
}
